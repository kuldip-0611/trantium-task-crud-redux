...
export const EditModal = ({ isOpen, setShowModal, id, posts, setPosts }) => {
    ...
    const onSubmit = (data) => {
        axios.put(`${apiUrl.postsUrl}/${id}`, {
            ...
            .then(data => setPosts(posts.map(post => post.id === id ? data : post)), toast.success(validation.editPost.success), setShowModal(false))
            .catch(error => {
                toast.error(validation.editPost.error)
            });
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Form.Control
                                        {...field}
                                        type="text"
                                        className={`border border-2 mt-2 ${errors.title ? 'is-invalid' : ''}`}
                                    />
                                )}
                            />
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Form.Control
                                        {...field}
                                        type="text"
                                        className={`border border-2 mt-2 ${errors.description ? 'is-invalid' : ''}`}
                                    />
                                )}
                            />
